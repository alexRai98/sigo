...
/***********************************************************/
// Creado por Alejandro Ruiz Robles
// sab 14 jun 2003 00:00 horas
// aruiz@correo.udep.edu.pe
/***********************************************************/
...
class OrejoTemplate {
	...
	function OrejoTemplate($archivo=""){
		if($archivo!=""){
	...
	function preparaPlantilla() {
		...
		if($indiceInicioCabezaBloque > 0){
			$bloqueNormal = substr($this->plantilla,0,$indiceInicioCabezaBloque); //obtengo bloque normal, está antes de empezar el bloque dinamico
			...
			if(strpos($bloqueDinamico,"<!-- INICIO BLOQUE : ")) {
				...
				$this->bloquesDinamicosCompuestosAux[$nombreBloque] = new OrejoTemplate();
	...
	function preparaPlantillaFinal() {
		for($i=0; $i<sizeOf($this->posicion); $i++){
			...
			if(isset($this->bloquesNormales[$this->posicion[$i]])){
				$this->plantillaFinal[] = str_replace('{([-_A-Za-z0-9]*)}',"",$this->bloquesNormales[$this->posicion[$i]]);
			}
			...
			if(isset($this->bloquesDinamicosSimples[$this->posicion[$i]])){
				for($j=0; $j < sizeOf($this->bloquesDinamicosSimplesTemporales[$this->posicion[$i]]); $j++){
					$this->plantillaFinal[] = str_replace('{([-_A-Za-z0-9]*)}',"",$this->bloquesDinamicosSimplesTemporales[$this->posicion[$i]][$j]);

				}
				$this->plantillaFinal[] = str_replace('{([-_A-Za-z0-9]*)}', "", $this->bloquesDinamicosSimplesAux[$this->posicion[$i]]);
			}
			...
			if(isset($this->bloquesDinamicosCompuestos[$this->posicion[$i]])){
				for($j=0; $j<sizeOf($this->bloquesDinamicosCompuestosTemporales[$this->posicion[$i]]); $j++){
					$this->plantillaFinal[] = str_replace('{([-_A-Za-z0-9]*)}',"",$this->bloquesDinamicosCompuestosTemporales[$this->posicion[$i]][$j]);

				}
				$tmp = $this->bloquesDinamicosCompuestosAux[$this->posicion[$i]]->getPlantillaArray();
				for($j=0; $j<sizeOf($tmp); $j++){
					$this->plantillaFinal[] = str_replace('{([-_A-Za-z0-9]*)}',"",$tmp[$j]);
				}
			}
		}
	}
